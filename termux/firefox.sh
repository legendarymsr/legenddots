...
mkdir -p "$BIN_DIR"
WRAPPER="$BIN_DIR/firefox-esr"
# clean up the old 'firefox' name if a previous run left one
rm -f "$BIN_DIR/firefox"
say "Adding a 'firefox-esr' launcher command at $WRAPPER (so rofi/Alt+D lists it)..."
cat > "$WRAPPER" <<EOF
#!/usr/bin/env bash
...
:: Done. To run Firefox ESR inside pocketwl:
     * from a terminal INSIDE pocketwl: run  firefox-esr
     * or pick "firefox-esr" from rofi (Alt+D)
