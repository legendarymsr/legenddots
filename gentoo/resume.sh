...
if [[ -f /mnt/gentoo/etc/gentoo-install.state ]] && grep -qx kernel /mnt/gentoo/etc/gentoo-install.state; then
  ...
  if [[ "$KMAJOR" =~ ^[0-9]+$ ]] && (( KMAJOR >= 7 )); then
    ...
    && ! grep -qE '^CONFIG_DRM_FBDEV_EMULATION=y' /mnt/gentoo/usr/src/linux/.config; then
    # Kernel was built without framebuffer console support — nomodeset boots
    # silently with no display output.  Rebuild with the updated config.
    # finalize is also dropped because broadcom-sta's wl.ko must be recompiled
    # against the new kernel; finalize re-emerges it.
    echo -e "${CYAN}Stale kernel config detected (missing framebuffer console) — forcing kernel + finalize rebuild...${NC}"
    sed -i '/^kernel$/d; /^finalize$/d' /mnt/gentoo/etc/gentoo-install.state
  fi
fi
...
if [[ -f /mnt/gentoo/etc/gentoo-install.state ]] && grep -qx finalize /mnt/gentoo/etc/gentoo-install.state; then
  if [[ ! -f /mnt/gentoo/boot/refind_linux.conf ]] \
    ...
    echo -e "${CYAN}Stale finalize state detected (refind_linux.conf missing, has nomodeset, or lacks i915 params) — forcing finalize to rerun...${NC}"
