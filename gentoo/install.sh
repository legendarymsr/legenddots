...
if ! step_done kernel; then
  ...
  # simpledrm reads EFI GOP framebuffer params (incl. Apple's non-standard
  # stride) and is the only display driver that works with nomodeset on EFI-
  # only machines.  The three options below it wire up the console pipeline:
  # fbdev emulation exposes /dev/fb0 from the DRM device, SYSFB_SIMPLEFB
  # registers the EFI framebuffer as a platform device for simpledrm to claim,
  # and FRAMEBUFFER_CONSOLE puts the kernel text console on that framebuffer.
  # Without all four built-in (=y), nomodeset boots silently with no display.
...
if ! step_done finalize; then
  ...
  # Rebuild broadcom-sta against the current kernel.  wl.ko is compiled
  # against the kernel's Module.symvers; if the kernel was rebuilt (config
  # change, etc.) the old wl.ko is stale and modprobe fails silently.
...
EOF
  ...
  # usbmouse can race bcm5974 and claim the trackpad interface first — blacklist it.
  # Standard USB mice are handled by usbhid, not usbmouse.
...
EOF
  ...
  # rEFInd works natively with Apple EFI.  --usedefault writes to the EFI
  # fallback path (/EFI/BOOT/BOOTX64.EFI) so Apple firmware always finds it.
  # sys-boot/refind may already be installed from a prior run; emerge is a
  # no-op if so.
  ...
  # rEFInd auto-detects vmlinuz but misses genkernel's initramfs naming
  # (the version infix breaks its heuristics), so without this file it
  # boots the kernel bare and panics at root mount.
  #
  # acpi_osi=   : stop Apple firmware activating Mac-specific ACPI paths
  #               that conflict with Linux power management.
  # pcie_aspm=off : PCIe power management causes spurious hangs on Apple EFI.
  # NOTE: nomodeset is intentionally absent — it would disable i915 KMS and
